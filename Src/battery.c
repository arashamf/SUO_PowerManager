...
void ChargeBattery(void)
{
	...
	if(flag_switch_off == 1) {extLEDstate=EXT_LED_OFF;} //Устройство выключено, хотя и под сетевым питанием
	else			 {extLEDstate=EXT_LED_ON; } //Устройство включено, под сетевым питанием
...
void UseBattery(void)
{
	...
	if(fShutDown==1) {		// no matter what happen, we will shut down soon.
		...
		cnt++;
		if((cnt>(DELAY_BEFORE_SW_OFF_BAT*1000))||(avgValue.vBat<BAT_LEVEL_CRITICAL)) {	// 1 minute for the host shutting down, but sooner if voltage gets critical
			CHARGE_ENABLE(OFF);
			V24_ENABLE(OFF);
			BAT_ENABLE(OFF);
			Generate36V(OFF);
